...
%     [Y,lambda,A,Xs] = Xpca(X,30);        % 30 principal components
%     figure(1);bar(lambda/lambda(1))
%     figure(2);imshow([X Xs],[])
%
%  Example 2:
%     X = double(imread('cameraman.tif')); % 256x256 pixels
%     [Y,lambda,A,Xs] = Xpca(X,0.90);      % 90% of energy
%     figure(1);bar(lambda/lambda(1))
%     figure(2);imshow([X Xs],[])

function [Y,lambda,A,Xs,mx,B] = Xpca(X,d,options)
