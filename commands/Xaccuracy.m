...
%
%    Xaccuracy(d1,d2) and Xaccuracy(d2,d1) obtain the same result.
%
%    Example:    
%       load datagauss           % simulated data (2 classes, 2 features)
%       Xplotfeatures(X,d)       % plot feature space
%       op.p = [];
%       ds   = Xlda(X,d,Xt,op);  % LDA classifier
%       p    = Xaccuracy(ds,dt)  % performance on test data

function p = Xaccuracy(d1,d2,nn)
...
if (n1==1) || (n2==1)
    ...
    for i=1:n
        if exist('nn','var');
            [~,pp] = Xconfusion(d,ds(:,i),nn);
        else
            [~,pp] = Xconfusion(d,ds(:,i));
        end
        p(i) = mean(pp);
    end

else
    error('Xaccuracy: at least d1 or d2 must have only one column.');
end