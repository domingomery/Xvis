...
%    figure(1)
%    Xplotfeatures(X(:,1),d,'x1')      % histogram of feature 1
%    figure(2)
%    Xplotfeatures(X,d)                % plot feature space in 2D (2 features)
%
% Example 2: 3D
%    load datareal                     % real data 
%    X = f(:,[221 175 235]);           % only three features are choosen
%    Xplotfeatures(X,d)                % plot feature space in 3D (3 features)
%
...
%    Xplotfeatures(f(:,s),d)           % plot feature space for 5 features
%

function Xplotfeatures(X,d,Xn)
...
if m>9
    error('Xplotfeatures for %d features makes %d plots.',m,m*m)
end
...
%                 max_X = max(X1);
%                 
%                 [~,pdf1,x1] = kde(X1,128,min_X,max_X);
%                 plot(x1,pdf1,col(k+1));
%                 hold on
