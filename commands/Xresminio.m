% Fs = Xresminio(G,h,method)
%
% Toolbox Xvis: Image restoration using MINIO criterium.
%
%  G : blurred image
%  h : PSF
...
%
%  Example:
%     F = double(imread('circuit.tif'));       % original image
%     n = 45;                                  % amount of blur
%     h = ones(1,n)/n;                         % PSF
%     G = conv2(F,h,'valid');                  % degradation
...
function Fs = Xresminio(G,h,method)
if ~exist('method','var')
    method = 1;
end
n = length(h);
